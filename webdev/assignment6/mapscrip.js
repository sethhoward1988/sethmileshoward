...
var createSettingsPopup, populateCameraForm,
...
// Google Earth plugin management functions
    suggestCameraObject, clearOldAndLoadNewPlacemarks, clearPlacemarks,
 ...
 */
    ...
    libraryObject = {
        ...
        populateCameraForm: function () {
            var currentRows = $('#cameraform td.oldValue'),
                newRows = $('#cameraform td.get'),
                camera = ge.getView().copyAsCamera(ge.ALTITUDE_RELATIVE_TO_GROUND);
            ...
            $.each(currentRows, function(index, row) {
                ...
                $(row).text(MS.currentLocation[index]);
                suggestCameraObject[classes[1]] = MS.currentLocation[index];
            });

            $.each(newRows, function(index, row){
                var classes = $(row).attr('class').split(' '),
                    value = eval('camera.get' + classes[classes.length - 1] + '()');
                    
                //I would probably never use eval, and find a better way to do this, since you could inject a class name
                //that is javascript code and make some injected server calls. Eval is Evil. But sometimes quick.
                $(row).text(value);
                suggestCameraObject['view' + classes[classes.length - 1]] = value;
            });
        }
    ...
    $(function() {
        ...
        $('#cameraform .submit.button').on('click', function() {
           $.post('http://scriptures.byu.edu/mapscrip/suggestpm', suggestCameraObject) 
           libraryObject.removePopup();
        });
...
function showLocation(geotagId, placename, latitude, longitude,
    ...
    if($('#rememberCamera').css('display') == 'none'){
        this.currentLocation = placename;
        $('#rememberCamera').fadeIn();
    }
